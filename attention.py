...
def process_attention_weights(attention_weights):
    ...
    normalized_attention_weights = (attention_weights_head_0 - min_values) / (max_values - min_values + 1e-10)  # 加上一个小的值防止除以零
    ...
    aggregated_attention_normalized = (aggregated_attention - min_agg_value) / (max_agg_value - min_agg_value + 1e-10)  # [1, 114]
    ...
    output_file_path = os.path.join(output_dir, f"new_weights_21_{global_file_index}.txt")
    ...
    attention_values = aggregated_attention_normalized.cpu().detach().numpy().flatten()  
...
class GAT(nn.Module):
    ...
    def forward(self, x, adj,n_layers):
        ...
        for i in range(n_layers):
            ...
            h1 = h.repeat(1,1, N)          # h:(1,78,34)   h1:(1,78,34*78)
            h2 = h1.view(size, N * N, -1)   # h2:(1,78*78,34)  78*78=6084    (78*34*78)/(78*78)=34
            h3 = h.repeat(1, N, 1)          # h3:(1,78*78,34)
            ...
            # print(f"Layer {i+1} Attention Weights Shape: {attention.shape}")
            # print(attention)
...
class InteractionModel(nn.Module):
    ...
    def forward(self, compound_features, protein_features):
        ...
        compound_attention_output,  compound_attention_weights  = self.compound_attention(compound_embedded, compound_embedded,
                                                               compound_embedded)
        protein_attention_output, protein_attention_weights = self.protein_attention(protein_embedded, protein_embedded, protein_embedded)
        ...
        self.attention_weights = {  
            'compound': compound_attention_weights.detach().cpu().numpy(),  
            'protein': protein_attention_weights.detach().cpu().numpy()  
        }  
...
class Predictor(nn.Module):
    ...
    def __init__(self,gat,cnn, decoder,device,n_fingerprint,n_layers,atom_dim=34):
        super().__init__()
    ...
    def forward(self, compound1, compound2,adj, protein1,protein2,n_layers):
        ...
        compound2 = self.gat(compound2, torch.unsqueeze(adj,0),n_layers)
